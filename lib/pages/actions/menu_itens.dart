    ...
    return (Container(
      ...
      child: SingleChildScrollView(
        ...
        child: Wrap(
          ...
          children: [
            _itemMenu(icon: MdiIcons.clover, name: 'Área Pix'),
            _itemMenu(icon: MdiIcons.barcode, name: 'Pagar'),
            _itemMenu(icon: MdiIcons.cash, name: 'Transferir'),
            _itemMenu(icon: MdiIcons.bankOutline, name: 'Depositar'),
            _itemMenu(icon: MdiIcons.creditCardSyncOutline, name: 'Recarga'),
  ...
  _itemMenu({required IconData icon, required String name, bool ? isFirst}) {
    return Container(
      padding: (isFirst ?? false)
          ? const EdgeInsets.only(right: 7, left: 30)
